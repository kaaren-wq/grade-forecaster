...
struct UofTCalculatorView: View {    @State private var showCelebration = false
        ...
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                VStack(alignment: .leading, spacing: 8) {
                    Text("UofT Grade Calculator")
                        .font(.system(size: 30, weight: .bold))
                        .foregroundColor(.white)
                    
                    Text("Grade predictor for UofT students")
                        .font(.subheadline)
